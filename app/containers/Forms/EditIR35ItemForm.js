...
function EditIR35ItemForm() {
  ...
  const getData = () => {
    axios({
      method: "GET",
      url: URL + "getIR35ItemFormDataWithID/" + getCookie("editDataId"),
    })
      .then((res) => {
        if (res.data.status == 100) {
          ...
          setData({
            ...
            category_id: res.data.item.category_id,
            category_Item_id: res.data.item.calculation_method,
            min_score: res.data.item.min_score,
            max_score: res.data.item.max_score,
            is_max_default: res.data.item.is_max_default,
          });
          loading(false);
          setForm(res.data);
        }
  ...
  const submit = () => {
    ...
    axios({
      method: "POST",
      url: URL + "editNewIR35Item/" + getCookie("editDataId"),
      data: data,
      headers: {
  ...
  return (
    <div>
      ...
      <PapperBlock
        title="Edit IR35 Category Item"
        icon="ion-ios-list-box-outline"
        desc="Fillout the required information below to update the Category Item for IR35 Driver Score"
      >
        <div>
          <Grid
            container
...
export default withStyles(styles)(EditIR35ItemForm);
