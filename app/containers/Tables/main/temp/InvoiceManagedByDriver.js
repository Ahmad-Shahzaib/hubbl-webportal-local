...
function InvoiceManagedByDriver(props) {
  ...
  function getData(limit) {
    let url =
      ...
      getCookie("userType") +
      "/" +
      "Invoice%20managed%20by%20the%20Driver";
    if (limit) {
      url += "/" + limit;
    }
  ...
  function onEdit() {
    if (dataIDs.length == 1) {
      ...
      setCookie("redirect", "invoice-managed-by-the-driver");
  ...
  return (
    <div>
      ...
      <PapperBlock
        whiteBg
        icon="ion-ios-paper"
        title="HGV Driver Hub - Invoice Managed By The Driver"
        desc="Invoice Managed By The Driver Timesheet List"
      >
        <Grid
          xs={12}
          md={12}
...
export default withStyles(styles)(InvoiceManagedByDriver);
