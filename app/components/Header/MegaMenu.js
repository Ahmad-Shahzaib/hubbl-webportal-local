...
function MegaMenu(props) {
  ...
  const getMenus = (parent, menuArray) =>
    menuArray.map((item, index) => {
      if (item.can_access !== undefined && !item.can_access) {
        return false;
      }
      if (item.multilevel) {
        return false;
          ...
          <div key={index.toString()}>
            {item.linkParent && item.key == "chat" ? (
              <Button
                ...
                onClick={(event) =>
                  window.open(item.linkParent + getCookie("email"))
                }
              >
                {item.name}
                {!item.linkParent ? (
            ...
            ) : item.can_access == undefined || item.can_access ? (
              <Button
                aria-haspopup="true"
                        ...
                        <Grid container>
                          <Grid item md={3} container justify="center">
                            ...
                            <span className={classes.bigIcon}>
                              <i
                                className={classNames(classes.icon, item.icon)}
                              />
                            </span>
                          </Grid>
                          <Grid item md={9}>
  ...
  return (
    <nav className={classes.mainMenu}>
      <div className={classes.megaMenu}>{getMenus(null, dataMenu)}</div>
    </nav>
  );
}
