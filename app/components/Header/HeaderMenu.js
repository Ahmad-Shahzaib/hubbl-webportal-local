...
import logo_white from "dan-images/logo_white.png";
...
function HeaderMenu(props) {
  ...
  const turnMode = (mode) => {
    if (mode == "light") {
      setCookie("theme", "dark", 87660);
      props.changeMode("dark");
    } else {
      setCookie("theme", "light", 87660);
  ...
  useEffect(() => {
    if (getCookie("theme")) {
      props.changeMode(getCookie("theme"));
    }
  }, []);
        ...
        <Hidden smDown>
          ...
          <NavLink to={logoLink} className={classes.brand}>
            <img
              ...
              alt={brand.name}
              style={platformConfig.logoStyle}
            />
            {/* {brand.name} */}
          </NavLink>
        </Hidden>
